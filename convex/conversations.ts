...
export const createConversation = mutation({
    ...
    handler: async (ctx, args)=> {
        ...
        let groupImage = args.groupImage;
...
export const deleteGroup = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!conversation) throw new ConvexError("Conversation not found");
    if (!conversation.isGroup)
      throw new ConvexError("Only group conversations can be deleted");

    if (conversation.admin !== user._id)
      throw new ConvexError("Only the group admin can delete the group");
    ...
    if (conversation.groupImage) {
      await ctx.storage.delete(conversation.groupImage);
    }
